       ...
       if(comoperfil.getSelectedIndex()>=0){
        ...
        if(acceso.EntrarSistema(sql).equals("1")){dispose();
         ...
         + " on e.id_persona=p.id_persona inner join usuarios u on u.id_persona=p.id_persona "
          ...
          MenuSistema.labeldni.setText(controlbase.DevolverDatosSql("select p.dni from persona p inner join empleados e "
        + " on e.id_persona=p.id_persona inner join  usuarios u on u.id_persona=p.id_persona "
...
public void PermisoUsuario(){
   ...
   if(perf.equals("Administrador")){menu.procesos.setEnabled(false);
   }else{if(perf.equals("Vendedor")){menu.Reportes.setEnabled(false);
   menu.itemconsultausuarios.setEnabled(false);menu.itemclientes.setEnabled(false);
   ...
   menu.itemtrabajadores.setEnabled(false);}}
